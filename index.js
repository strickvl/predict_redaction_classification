...
function onFileSelected(event) {
  ...
  reader.addEventListener("loadend", function () {
    ...
    fetch(
      "https://hf.space/embed/strickvl/fastai_redaction_classifier/+/api/predict/",
      {
        method: "POST",
      ...
      .then(function (response) {
        if (response.status != 200) {
          ...
          errorEl.innerHTML =
            "<u>Sorry the API is not working currently. Please try again later</u>";
      ...
      .then(function (json_response) {
        ...
        const firstLabelConfidence =
          json_response?.data[0]?.confidences[0]?.confidence;
        const secondLabel = json_response?.data[0]?.confidences[1]?.label;
        const secondLabelConfidence =
          json_response?.data[0]?.confidences[1]?.confidence;
